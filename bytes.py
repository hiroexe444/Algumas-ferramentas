...
def clean_hex(s: str) -> bytes:
    ...
    # FIX: precedência corrigida com parênteses explícitos
    # Antes: len >= 8 and fullmatch OR ratio >= 0.6  →  bug
    # Agora: testa fullmatch primeiro (hex puro), depois ratio como fallback
    is_pure_hex = len(hex_chars) >= 8 and bool(re.fullmatch(r'(?:[0-9a-fA-F]{2}\s*)+', s))
    s_no_space = s.replace(" ", "")
    is_mostly_hex = len(s_no_space) > 0 and (len(hex_chars) / len(s_no_space)) >= 0.6

    if is_pure_hex or is_mostly_hex:
        if len(hex_chars) % 2 != 0:
            hex_chars = '0' + hex_chars
        return bytes.fromhex(hex_chars)

    # trata como raw ascii payload
